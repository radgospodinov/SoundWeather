		...
		<table class="table_with_own_sounds">
			<c:forEach var="sound" items="${requestScope.sounds}">
				<tr id="s${sound.getSoundId()}">
					<td><img class="own_sound_cover_photo" alt=""
						src="<c:url value="/covers/${sound.getFileName()}.jpg"/>"
					...
					<td><select id="add_to_album${sound.soundId}" name="album">
							<c:forEach items="${requestScope.albums}" var="album">
								<option value="${album.albumId}"><c:out
										value="${album.albumTitle}" /></option>
							</c:forEach>
					</select></td>
					<td>
						<button id="sound_id_to_delete"
